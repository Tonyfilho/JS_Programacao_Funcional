...
export class ReduceComponent implements OnInit {
  ...
  lancamentos = this.rockets.reduce(
    (newValue, actualArray) => newValue + actualArray.launches,
    0
  );
  ...
  sexoMouF = this.pessoas.reduce(
    (newValue: any, actualArray: any) => {
  ...
  sexoMouF2 = this.pessoas.reduce(
    (newArray: any, actualArray: any) => {
      actualArray.sexo === 'M'
        ? newArray.M?.push(actualArray)
        : newArray.F?.push(actualArray);
      return newArray;
    },
    { M: [], F: [] }
  );
  ...
  sexoMouF3 = this.pessoas.reduce(
    (newValue: any = '', accurateArray: any) => {
      accurateArray.sexo === 'M'
        ? newValue['M'].push(accurateArray)
        : newValue['F'].push(accurateArray);
      return newValue;
    },
    { M: [], F: [] }
  );

  maiorDe18 = this.pessoas.reduce(
    (newArray: any, actualArray) => {
  ...
  maioresDe18Maculinos = this.pessoas.reduce(
    function (newArray: any, actualArray) {
      actualArray.idade >= 18 && actualArray.sexo === 'M'
        ? newArray.maiorEMasculino.push(actualArray)
        : '';
      return newArray;
    },
    { maiorEMasculino: [] }
  ...
  maioresDe18Femeas = this.pessoas.reduce(
    (newArray: any, actualPessoas) => {
      actualPessoas.idade >= 18 && actualPessoas.sexo === 'F'
        ? newArray.maioresDe18Femeas.push(actualPessoas)
        : '';
      return newArray;
    },
    { maioresDe18Femeas: [] }
  ...
  maioresEMenores = this.pessoas.reduce(
    (newArray: any, actualArray) => {
  ...
  masculinoOuFeminino = this.pessoas.reduce(
    (newArray: any, actualArray) => {
  ...
  exercicioMascOuFem = this.pessoas.reduce(
    (acumulador: any, atual) => {
